...
class TicTacToe:
    ...
    def minimax(self, depth, max_player, alpha, beta):
        if depth == 0 or self.has_won('X') or self.has_won('O') or self.is_full():
            return 1000 if self.has_won('X') else (-1000 if self.has_won('O') else 0)
        if max_player:
            max_eval = -math.inf
            for col in range(6):
                for row in range(5):
        ...
        for col in range(6):
            for row in range(5):
                if self.board[row][col] == ' ' and self.is_valid_successor(col, row, player):
                    ...
                    eval = self.minimax(depth, player != 'X', -math.inf, math.inf)
                    self.board[row][col] = ' '
...
def play_tic_tac_toe():
    ...
    while not game.finished:
        game.display()
        if current_player:
            y, x = game.find_best_move('X', 2)
            if (x, y) is None:
                print("No valid move found! Exiting game.")
                return
            game.successor(x, y, 'X')
        else:  # Computer's turn
            y, x = game.find_best_move('O', 4) 
            if (x, y) is None:
                print("No valid move found! Exiting game.")
                return
            game.successor(x, y, 'O')
